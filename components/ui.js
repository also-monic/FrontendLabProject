export default class UI{
    ...
    draw(context){
        ...
        //score
        context.fillText('Highest Score : '+ this.game.highestScore , 20,40);
        context.fillText('Score : '+ this.game.score , 20,70);

        // game timer
        context.font = '30px Rubik Scribble';
        if(this.game.maxTime-this.game.time > 0)
            context.fillText('Time : '+ ((this.game.maxTime-this.game.time) * 0.001).toFixed(1) , 20,100);
        else
            context.fillText('Time : 0', 20,90);

        // lives
        for(let i = 0 ; i < this.game.lives ; i++){
            context.drawImage(this.livesImage, 20 * 1.5*i + 20 ,110 ,25,25);
        }
